...
def hexToDec(hexValue: str) -> int:
    """ Converts a given hexadecimal value to a decimal value """
    return int(hexValue, 16)

def parseColorsToMoves(data: list[str]) -> list[list[Union[str, int]]]:
    """ Given the list of hexadecimal color strings,
        transform them to moves as described in the assignment
    """
    moves = []
    hexToDir = {"0": "R", "1": "D", "2": "L", "3": "U"}
    for i in range(len(data)):
        # compute the distance in meters and the direction
        distPart = data[i][:-1]
        dirPart = data[i][-1]
        
        dist = hexToDec(distPart)
        dir = hexToDir[dirPart]
        
        # and add this move to the list
...
with open(INPUT_FILE, 'r') as f:
    ...
    for i in range(len(data)):
        # keep only the colors, removing the "(#" and ")"
        data[i] = data[i][2][2:-1]
    ...
    # 2) extract the moves from the colors
    moves = parseColorsToMoves(data)
    ...
    # 3) get all edge points
    edges = generateEdgesFromMoves(moves)
    ...
    # DEBUG
    if (DEBUG):
        print("START OF EDGES:")
        print(edges[:10])
    
    # 4) create the map with the border
    ...
    # 5) compute how many tiles are on the border and inside
    terrainSize = computeSpaceOccupiedBy(borders)
    # 6) give the result of first approach
    print(f"Approach 1: \nExactly {terrainSize} cubic meters of lava fit in the lagoon\n")
        
    # 7) compute the map where the inside is filled in
    ...
    # 8) count up the number of filled in spaces
    numberFilledInSpaces = sum([sum(row) for row in filledInMap])
    
    # 9) return the result for the second approach
